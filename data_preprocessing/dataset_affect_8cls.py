...
class Affect8DataSet(data.Dataset):
    def __init__(self, path, train=True, transform=None, basic_aug=False):
        self.train = train
        self.root = path
        self.transform = transform
        self.train_paths = os.path.join(self.root, 'train_set/annotations_8cls.txt')
        self.val_paths = os.path.join(self.root, 'val_set/annotations_8cls.txt')
        ...
        for f in file_names:
            if self.train:
                ...
                feature_path = os.path.join(self.root, 'train_set/feature', f.replace('.jpg', '_feature.jpg'))
            else:
                path = os.path.join(self.root, 'val_set/images', f)
                feature_path = os.path.join(self.root, 'val_set/feature', f.replace('.jpg', '_feature.jpg'))
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            if self.basic_aug and random.uniform(0, 1) > 0.5:
                ...
                feature_sample = self.aug_func[index](feature_sample)
...
if __name__ == "__main__":
    test = Affect7DataSet(path='../dataset/affect', train=False)
    test.__getitem__(1)
